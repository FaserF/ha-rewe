...
async def async_setup_entry(
    ...
    if coordinator.user_token:
        ...
        if coordinator.account_key not in created_buttons:
            ...
            async_add_entities(
                [ReweActivateCouponsButton(coordinator)], update_before_add=False
            )
...
class ReweForceUpdateButton(ButtonEntity):
    ...
    async def async_press(self) -> None:
        """Press the button."""
        _LOGGER.info("Force update button pressed for REWE market %s", self._market_id)
        self.coordinator._force_update = True
        await self.coordinator.async_request_refresh()


class ReweActivateCouponsButton(ButtonEntity):
    """Button to activate all available REWE Bonus coupons."""
    ...
    def __init__(self, coordinator: ReweDataUpdateCoordinator) -> None:
        ...
        self._attr_unique_id = f"rewe_{self._account_key}_activate_coupons"
        self._attr_device_info = DeviceInfo(
    ...
    async def async_press(self) -> None:
        _LOGGER.info("Activate all coupons button pressed for REWE account")
        await self.coordinator.async_request_refresh()
